...
// Get the search query from the search form (if any)
$query_search = '';
if (isset($_GET['query'])) {
    $query_search = $_GET['query'];
}

// Query to get all courses matching the search query with the forum post date
$query = "SELECT c.course_id, c.title, c.description, c.category, c.price, 
          ...
          ORDER BY c.title ASC";  // Sort alphabetically by title
$stmt = $conn->prepare($query);
$search_param = "%" . $query_search . "%"; // Search term wrapped with wildcards
$stmt->bind_param("s", $search_param);  // Only bind the search query to the title
$stmt->execute();
$result = $stmt->get_result();
