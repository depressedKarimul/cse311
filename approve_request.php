...
<body>
  ...
  $query = "SELECT * FROM User WHERE role = 'instructor' AND (is_approved IS NULL OR is_approved = 0)";
  ...
  <?php if (mysqli_num_rows($result) == 0): ?> <!-- Changed to 0 to check for no pending requests -->
      ...
      <?php while ($row = mysqli_fetch_assoc($result)): ?>
        <div class="w-full sm:w-1/3 lg:w-1/3 p-2">
          <div class="bg-[#283747] shadow-xl rounded-lg py-3">
            ...
            <div class="p-2">
              <h3 class="text-center font-extrabold text-3xl text-white"><?= htmlspecialchars($row['firstName'] . ' ' . $row['lastName']) ?></h3>
